...
public class DashboardView extends View {
    ...
    private int color = Color.parseColor("#00b4aa");
    ...
    protected void onDraw(Canvas canvas) {
         ...
         * 画指针
         */
        float realValue = mRealTimeValue;
        if(mRealTimeValue>mMax)
            realValue = mMax;
        if(mRealTimeValue<mMin)
            realValue = mMin;

        float θ = mStartAngle + mSweepAngle * (realValue - mMin) / (mMax - mMin); // 指针与水平线夹角
...
//            return;
//        }
        if(realTimeValue>mMax/2 && realTimeValue<mMax*2/3)
            color = Color.parseColor("#ff9900");
        else if(realTimeValue>=mMax*2/3)
            color = Color.parseColor("#ff3300");
        else if(realTimeValue>mMin*2/3 && realTimeValue<mMin/2)
            color = Color.parseColor("#00868B");
        else if(realTimeValue<=mMin*2/3)
            color = Color.parseColor("#0000CD");
        else if(realTimeValue>=mMin/3 && realTimeValue<mMax/2)
            color = Color.parseColor("#00b4aa");
